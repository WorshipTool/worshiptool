import { LOGIN_URL, BACKEND_URL, GETSONGBYGUID_URL, GETSONGQUERY_URL, POSTNEWSONG_URL, SIGNUP_URL, VERIFYSONG_URL, UNVERIFYSONG_URL, DELETESONG_URL, GETUNVERIFIED_URL, GETLOADERUNVERIFIED_URL, GETSONGCOUNT_URL, GETSONGLIST_URL, POSTMERGESONGS_URL, GETPLAYLISTS_URL, POSTCREATEPLAYLIST_URL, POSTADDTOPLAYLIST_URL, GETSONGSINPLAYLIST_URL, GETISVARIANTINPLAYLIST_URL, POSTREMOVEFROMPLAYLIST_URL, POSTDELETEPLAYLIST_URL, RESTOREVARIANT_URL } from './constants';
...
export function getUrl_RESTOREVARIANT(guid:string){
    const generatedURL = BACKEND_URL + RESTOREVARIANT_URL.replace(":guid", guid);
    return generatedURL;
}
