  ...
  return (
    <Container>
        ...
        <ButtonGroup variant="outlined" orientation='horizontal'>
            <Tooltip title={"Sloka"}><Button onClick={()=>{onNewSection("V")}}>V</Button></Tooltip>
            <Tooltip title={"Refrén"}><Button onClick={()=>{onNewSection("R")}}>R</Button></Tooltip>
            <Tooltip title={"Bridge"}><Button onClick={()=>{onNewSection("B")}}>B</Button></Tooltip>
        </ButtonGroup>
        <Gap value={0.5}/>
        <Typography variant='caption' justifyContent={"center"} display={"flex"}>{"Alt + V/R/B"}</Typography>

        <Gap value={2}/>
        
        <Tooltip title={"Vložit akord"}>
            <Button variant='contained' onClick={()=>{onNewChord()}} sx={{display:"flex", flexDirection:"column"}}>
                Vlož akord
