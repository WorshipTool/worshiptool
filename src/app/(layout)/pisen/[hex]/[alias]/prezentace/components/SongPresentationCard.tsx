...
interface SongPresentationCardProps {
...
export default function SongPresentationCard({ variant, song, sectionIndex = 0 }: SongPresentationCardProps) {
	// Sheet
	...
	const onVariantChange = useCallback((variant: ExtendedVariantPack) => {
		if (!variant) return

		const newSheet = new Sheet(variant.sheetData)
		setSheet(newSheet)
	...
	useChangeDelayer(variant, onVariantChange, [onVariantChange], 1000)
	...
	const sizeIsOkey = useCallback((): boolean => {
		...
		// @ts-ignore
		const boxY: number = lastSectionRef.current.offsetTop
			...
			>
				...
				{currentSection ? (
					<Box
						...
						ref={lastSectionRef}
					>
						{sectionPart(currentSection, size)}
					</Box>
				) : (
					<Box
						sx={{
